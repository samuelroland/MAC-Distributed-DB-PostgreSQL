...
La cohérence forte est le saint Graal des systèmes transactionnels. Dans PGSQL, on l'obtient principalement avec la réplication synchrone. A chaque fois qu’une écriture est faite sur le leader, ce dernier envoie les modifications aux répliques synchrones. Ces répliques doivent confirmer qu’elles ont bien appliqué les changements avant que le leader ne valide la transaction. Lorsque le client reçoit une confirmation, toutes les copies des données sont alignées.
...
- Futur des bases de données distribuées avec PGSQL.
